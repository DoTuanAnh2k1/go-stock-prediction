...
# auth.proto: api-svc/proto/auth/auth.proto is the SINGLE SOURCE OF TRUTH.
# The Java copy (auth-svc/src/main/proto/auth.proto) is generated by copy — never
# edit it by hand. proto-auth syncs the Java copy first, then regenerates Go stubs.
proto-auth: proto-auth-sync
	cd api-svc && protoc --go_out=. --go_opt=paths=source_relative \
		--go-grpc_out=. --go-grpc_opt=paths=source_relative \
		proto/auth/auth.proto

# Copy canonical auth.proto -> Java tree. Both share java_package + go_package
# options, so ONE file serves both build systems byte-for-byte.
proto-auth-sync:
	cp api-svc/proto/auth/auth.proto auth-svc/src/main/proto/auth.proto
	@echo "synced auth.proto -> auth-svc/src/main/proto/auth.proto"

# CI guard: fail if the two copies drift (someone hand-edited the Java copy).
proto-auth-check:
	@diff -u api-svc/proto/auth/auth.proto auth-svc/src/main/proto/auth.proto >/dev/null \
		&& echo "auth.proto in sync" \
		|| { echo "ERROR: auth.proto drift — edit api-svc/proto/auth/auth.proto, then run 'make proto-auth-sync'"; exit 1; }
