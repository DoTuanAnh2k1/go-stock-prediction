...
public interface UserMarketGroupRepository extends JpaRepository<UserMarketGroup, UserMarketGroupId> {
    ...
    @Query(value = "SELECT DISTINCT mgm.market_key FROM user_market_groups umg JOIN market_group_markets mgm ON mgm.group_id = umg.group_id WHERE umg.user_id = :userId", nativeQuery = true)
