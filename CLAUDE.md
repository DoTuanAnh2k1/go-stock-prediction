...
api-svc/pkg/server/api_simulation.go            # GET /api/simulation/leaderboard (filter market/algorithm/currency/search; sort_by+sort_dir; phân trang opt-in page/page_size — KHÔNG có page/page_size → trả full list để cli-svc không bị cắt; luôn kèm chart aggregates distribution + top_returns tính server-side trên full filtered set; rank = canonical return-rank, không đổi theo display sort), bots, trades, chart; PUT config; POST toggle/run
...
| `GET` | `/api/monitoring/bots` | Bot trading table phân trang/lọc/sort server-side — yêu cầu JWT; query `?page=<n>&page_size=<n>&market=<GOLD\|NASDAQ\|CRYPTO\|SP500>&algorithm=<substring>&search=<bot_id substring>&sort_by=<win_rate\|total_pnl\|return_pct\|profit_factor\|trades\|wins\|losses\|bot_id\|market\|algorithm>&sort_dir=<asc\|desc>`; default page=1, page_size=50 (max 200), sort_by=win_rate, sort_dir=desc; trả `{data:[{bot_id, market, algorithm, trades, wins, losses, breakeven, win_rate, total_pnl, return_pct, profit_factor}], total, page, page_size, total_pages, summary:{total_bots, active_bots, by_market:[...]}}` (summary là global, không bị filter); dùng chung cache "monitoring:bots:full" 30s với /overview |
