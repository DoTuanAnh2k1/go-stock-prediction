...
k8s runner = CronJob template trong `deploy/helm/cronjobs/templates/` (chart `cronjobs` độc lập). Compose runner = `ofelia` label hoặc in-app APScheduler (khi `SCHEDULER_ENABLED=true`).
