...
export default function Training() {
  ...
  const handleTrain = () => {
    train().then((ok) => vnsToast(ok ? 'Đã gửi yêu cầu huấn luyện' : 'Chưa gọi được API huấn luyện'));
  };
  ...
  return (
    <div className="content__inner fade">
          ...
          {running
            ? <div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '16px 0 6px', fontSize: 12 }}>
                  <span style={{ color: 'var(--text-3)' }}>Tiến độ</span>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                  <button className="btn btn--accent" onClick={handleTrain}><Icon name="play" size={13} />Huấn luyện lại tất cả</button>
                  <button className="btn">Dừng</button>
                </div>
              </div>
            : <div>
                <div style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>Không có tác vụ nào đang chạy.</div>
                <button className="btn btn--accent" onClick={handleTrain}><Icon name="play" size={13} />Huấn luyện ngay</button>
              </div>
          }
        </Panel>

        <Panel title="Hàng đợi huấn luyện" sub={D.trainJobs.length ? D.trainJobs.length + ' tác vụ' : '—'} flush>
          {D.trainJobs.length === 0
            ? <div className="empty">
                <div className="empty__icon"><Icon name="layers" size={18} /></div>
                <p>Không có tác vụ trong hàng đợi</p>
              </div>
            : D.trainJobs.map((j, i) => (
              <div key={i} className="lrow">
                <span className={`badge badge--${j.status === 'running' ? 'accent' : j.status === 'done' ? 'up' : 'muted'}`} style={{ minWidth: 74, justifyContent: 'center' }}>
                  {j.status === 'running' ? 'Đang chạy' : j.status === 'done' ? 'Hoàn tất' : 'Chờ'}
                </span>
      ...
      <div className="sec-head"><h2>Mô hình</h2><div className="line"></div></div>
      {D.algos.length === 0
        ? <div className="empty section-gap">
            <div className="empty__icon"><Icon name="layers" size={18} /></div>
            <p>Chưa có dữ liệu mô hình. Bấm huấn luyện để bắt đầu.</p>
          </div>
        : <div className="grid grid--kpis section-gap">
            {D.algos.map((a) => (
              <div className="panel" key={a.id} style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span className="badge badge--up">Đã huấn luyện</span>
                </div>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12 }}>
                    <KV k="MAE" v={a.mae} />
                    <KV k="Δ độ chính xác" v={<Chg pct={a.accDelta} />} />
                    <KV k="Cập nhật" v={a.trainedAt} />
                  </div>
                </div>
              </div>
            ))}
      ...
      <Panel title="Nhật ký huấn luyện" sub="thời gian thực" flush style={{ paddingBottom: 8 }}
        ...
        <div style={{ background: 'oklch(0.13 0.012 245)', padding: '12px 0', maxHeight: 320, overflowY: 'auto', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
          {D.trainLogs.length === 0
            ? <div style={{ padding: '20px 18px', color: 'var(--text-3)', fontSize: 12 }}>Chưa có nhật ký huấn luyện.</div>
