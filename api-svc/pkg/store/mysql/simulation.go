	...
	err := c.Db.Raw(`
		SELECT
			...
			(SELECT ps.total_value
			 ...
			 ORDER BY ps.snapshot_date DESC LIMIT 1) AS current_value
		FROM sim_bots b
		LEFT JOIN sim_sessions s ON s.id = (
			SELECT ss.id
			FROM sim_sessions ss
			WHERE ss.bot_id = b.id
			ORDER BY
				(ss.mode = 'live' AND ss.status = 'running') DESC,
				(SELECT COUNT(*) FROM sim_portfolio_snapshots sp WHERE sp.session_id = ss.id) DESC,
				ss.id DESC
			LIMIT 1
		)
		ORDER BY COALESCE(s.total_return_pct, -99999) DESC
	`).Scan(&rows).Error
	return rows, err
}
