...
func (c *Client) CreateCryptoPrice(ctx context.Context, p *modelsdb.CryptoPrice) error {
...
func (c *Client) UpsertCryptoPrice(ctx context.Context, p *modelsdb.CryptoPrice) error {
	return c.db(ctx).Where("coin_id = ? AND trading_date = ?", p.CoinID, p.TradingDate).
		Assign(p).
		FirstOrCreate(p).Error
}

// GetCryptoPricesByDateRange returns crypto prices for a coinID within a date range, ordered DESC.
func (c *Client) GetCryptoPricesByDateRange(ctx context.Context, coinID string, from, to time.Time) ([]modelsdb.CryptoPrice, error) {
	var prices []modelsdb.CryptoPrice
	err := c.db(ctx).Where("coin_id = ? AND trading_date BETWEEN ? AND ?", coinID, from, to).
...
func (c *Client) GetLatestCryptoPrice(ctx context.Context, coinID string) (*modelsdb.CryptoPrice, error) {
	var price modelsdb.CryptoPrice
	err := c.db(ctx).Where("coin_id = ?", coinID).
		Order("trading_date DESC").
		First(&price).Error
	if err != nil {
		return nil, err
...
func (c *Client) GetCryptoCoins(ctx context.Context) ([]modelsdb.CryptoPrice, error) {
	var coins []modelsdb.CryptoPrice
	subQuery := c.db(ctx).Model(&modelsdb.CryptoPrice{}).
...
func (c *Client) GetAllCryptoPricesForCoin(ctx context.Context, coinID string) ([]modelsdb.CryptoPrice, error) {
	var prices []modelsdb.CryptoPrice
	err := c.db(ctx).Where("coin_id = ?", coinID).Order("trading_date DESC").Find(&prices).Error
	return prices, err
}

// BulkCreateCryptoPredictions inserts multiple crypto predictions using CreateInBatches.
func (c *Client) BulkCreateCryptoPredictions(ctx context.Context, preds []modelsdb.CryptoPrediction) error {
...
func (c *Client) DeleteCryptoPredictionsBeforeDate(ctx context.Context, before time.Time) error {
...
func (c *Client) CreateCryptoPrediction(ctx context.Context, p *modelsdb.CryptoPrediction) error {
	return c.db(ctx).Create(p).Error
}
...
func (c *Client) GetCryptoPredictions(ctx context.Context, coinID, algorithm string, limit int) ([]modelsdb.CryptoPrediction, error) {
...
func (c *Client) GetLatestCryptoPredictions(ctx context.Context) ([]modelsdb.CryptoPrediction, error) {
	var preds []modelsdb.CryptoPrediction
	subQuery := c.db(ctx).Model(&modelsdb.CryptoPrediction{}).
...
// Uses MAX(id) to avoid duplicates when multiple rows share the same prediction_date.
func (c *Client) GetLatestConfirmedCryptoPredictions(ctx context.Context) ([]modelsdb.CryptoPrediction, error) {
	var preds []modelsdb.CryptoPrediction
	subQuery := c.db(ctx).Model(&modelsdb.CryptoPrediction{}).
		Select("MAX(id) as max_id").
		Group("coin_id, algorithm_name")
...
func (c *Client) GetCryptoPredictionsByDateRange(ctx context.Context, coinID string, from, to time.Time) ([]modelsdb.CryptoPrediction, error) {
	var preds []modelsdb.CryptoPrediction
...
func (c *Client) GetCryptoPredictionsPage(ctx context.Context, page, limit int, search, algorithm, status, sortBy, sortDir string) ([]modelsdb.CryptoPrediction, int64, error) {
	var preds []modelsdb.CryptoPrediction
	var total int64
	...
	query := c.db(ctx).Model(&modelsdb.CryptoPrediction{})
