...
A real 2-layer LSTM. Supports per-market model caching via train().
...
class LSTMPredictor(PredictionAlgorithm):
    ...
    def train(self, prices: list[float], volumes: list[float] | None = None) -> None:
        ...
        try:
            ...
            log.info("lstm.trained", data_points=len(prices))
        except Exception as exc:
            log.warning("lstm.train_failed", error=str(exc))

    def train_batch(self, series: list[tuple[list[float], "list[float] | None"]]) -> None:
        """Train a single LSTM model on all price series combined."""
        try:
            import torch
            import torch.nn.functional as F
            ...
            if len(all_X) < 10:
                log.warning("lstm.batch_train_skip", reason="insufficient sequences", sequences=len(all_X))
                return
            ...
            log.info("lstm.batch_trained", series=len(series), sequences=len(all_X))
        except Exception as exc:
            log.warning("lstm.batch_train_failed", error=str(exc))
    ...
    def predict(self, prices: list[float], volumes: list[float] | None = None) -> PredictionResult:
        ...
        if self._model is not None:
            try:
                ...
                log.warning("lstm.inference_failed", error=str(exc))
    ...
    @staticmethod
    def _build_model():
        """Build a fresh _LSTM model instance."""
        try:
